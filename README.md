...
- **`@do-done/shared`** — `TaskSchema`, `ProjectSchema`, `LocationSchema`, `UserPreferencesSchema`, … plus enums (`TaskStatus`, `TaskPriority`); constants (`PRIORITY_CONFIG`, `STATUS_CONFIG`, `FOCUS_SCORES`); utils (`todayLocalISO()`, `isOverdue()`, `formatTimeOfDay()`, `resolveQuickSchedule()`); and the sort/group/filter `display` engine shared by web + mobile.
