 ...
 * for that day (when_date matches), or a calendar event from Google.
 *
 ...
 *
 * Tasks with no when_date are skipped (they don't appear on the calendar).
...
export function groupTasksByDate(tasks: Task[]): Map<string, BusyItem[]> {
  ...
  for (const t of tasks) {
    ...
    const existing = byDate.get(t.when_date);
    if (existing) {
      existing.push(item);
    } else {
      byDate.set(t.when_date, [item]);
    }
  }
  return byDate;
}
...
export class BusynessApi {
  ...
  async getTasksRange(
    ...
    let query = this.supabase
      .from("tasks")
      .select(
        "id, title, when_date, duration_minutes, priority, status, depth"
      )
